...
public class KCFABManager: NSObject {
    ...
    var fabWindow: KCFABWindow {
        get {
            if _fabWindow == nil {
                _fabWindow = KCFABWindow(frame: CGRect(origin: CGPoint(x: 0, y: 0), size: CGSize(width: UIScreen.mainScreen().bounds.width, height: UIScreen.mainScreen().bounds.height - 100)))
